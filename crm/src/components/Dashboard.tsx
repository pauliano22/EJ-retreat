import { TrendingUp, Users, Home, Phone, ArrowRight, Star } from 'lucide-react';
...
interface DashboardProps {
  ...
  onNavigate: (view: 'pipeline' | 'owners' | 'outreach' | 'owner-detail', extra?: string) => void;
}
...
export default function Dashboard({ leads, owners, outreach, onNavigate }: DashboardProps) {
  ...
  const totalMonthlyRevenue = activeProperties.reduce((sum, p) => sum + p.monthlyRevenue, 0);
  const avgOccupancy = activeProperties.length
  ...
  const stats = [
    {
      ...
      sub: `across ${activeProperties.length} active properties`,
      icon: TrendingUp,
      color: 'bg-teal-600',
    },
    {
      ...
      sub: `${owners.length} total in portfolio`,
      icon: Users,
      color: 'bg-indigo-500',
    },
    {
